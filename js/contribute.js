...
// Add link form `url` input handler
selectors.addLinkUrlInputEl.addEventListener('keyup', (event) => {
  console.log('--- keyup', event);
  event.preventDefault();
  showAddLinkFormSpinner();
  fetch.linkMetadata(event.target.value)
    .then(
      (metadata) => {
        ...
        if (imgUrl && imgUrl.indexOf('https://') === 0) {
          ...
          img.src = imgUrl;
          img.width = '250';
          selectors.addLinkImgContainerEl.appendChild(img);
        } else {
          hideAddLinkFormSpinner();
        }
      },
      (err) => {
        console.error(err.message);
        showAddLinkFormError(err.message);
        hideAddLinkFormSpinner();
      }
    );
});
...
selectors.addLinkFormEl.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    // TODO: validate that linkUrl returned 200 and title exist, minimally
    const signedData = {
      linkUrl: selectors.addLinkUrlInputEl.value,
      title: selectors.addLinkTitleInputEl.value.slice(0, 160),
      author: selectors.addLinkAuthorInputEl.value.slice(0, 50) || '',
      image: selectors.addLinkImgContainerEl.firstChild ? selectors.addLinkImgContainerEl.firstChild.src : '',
      timestamp: Date.now()
    }
    // Light form validation
    if (!signedData.linkUrl || !signedData.linkUrl.match(/^(https?):\/\/[^\s$.?#].[^\s]*$/)) {
      return showAddLinkFormError('Invalid link url. Try again.');
    }
    if (!signedData.title || signedData.title.length < 5) {
      return showAddLinkFormError('Please enter a title for the link. Minimum 5 characters.');
    }
    // Get user signature and post the form
    const signature = await getSignature(signedData);
    showAddLinkFormSpinner();
    const postResponse = await fetch.addLinkFormPost(signedData, account, signature);
    // Only success case is handled here
    // Failure handled in catch() below
    console.log('submit post response:', postResponse);
    showAddLinkFormSuccess('Success! Thanks for contributing.');
    hideAddLinkFormSpinner();
    showAddLinkFormError('');
    setTimeout(() => {
      resetAddLinkForm();
    ...
    if (err.message === 'Disconnected') {
      showAddLinkFormError('Please connect to Metamask.');
      connect();
    } else {
      showAddLinkFormError(err.message);
      hideAddLinkFormSpinner();
...
function resetAddLinkForm() {
  clearAddLinkFormInput();
  clearAddLinkFormMetadataFields();
  showAddLinkFormSuccess('');
  showAddLinkFormError('');
}

function clearAddLinkFormInput() {
  selectors.addLinkUrlInputEl.value = '';
}

function clearAddLinkFormMetadataFields() {
  selectors.addLinkTitleInputEl.value = '';
  selectors.addLinkAuthorInputEl.value = '';
  selectors.addLinkImgContainerEl.innerHTML = '';
}

function showAddLinkFormSpinner() {
  const spinnerEl = `<div class="spinner">
    <i class="fa fa-circle-o-notch fa-spin" aria-hidden="true"></i>
    </div>`;
  selectors.addLinkImgContainerEl.innerHTML = spinnerEl;
}

function hideAddLinkFormSpinner() {
  selectors.addLinkImgContainerEl.innerHTML = '';
}

// Pass in empty string to hide/ reset
function showAddLinkFormError(errMsg) {
  selectors.addLinkFormErrorEl.innerHTML = domPurify.sanitize(errMsg);
}

// Pass in empty string to hide/ reset
function showAddLinkFormSuccess(errMsg) {
  selectors.addLinkFormSuccessEl.innerHTML = domPurify.sanitize(errMsg);
}
...
function connect() {
  try {
    ...
    .then(() => {
      ...
      showAddLinkFormError('Click "Submit" again now.');
    })
    .catch((err) => {
      if (err.code === 4001) {
        // EIP-1193 userRejectedRequest error
        // If this happens, the user rejected the connection request.
        showAddLinkFormError('Please connect wallet if you want to submit a link.');
      } else {
        showAddLinkFormError(err.message);
      }
    });
    ...
    console.error(err.message);
    showAddLinkFormError('Please install Metamask or another Web3 wallet');
  }
}
...
async function getSignature(signedDataJson) {
  return new Promise(async (resolve, reject) => {
    const d = new Date();
    let address;
    try {
...
module.exports = {
  ...
  // Inputs
  addLinkUrlInputEl: document.getElementById('js-input-add-link'),
